...
\textbf{Verified Result:} $\beta = 9.07 \times 10^{-4}$ ($p=0.043$, N=47). \\
...
\end{tabular}
\end{center}
\textbf{Placebo (Chemicals NIC-20):} $\beta = -9.18 \times 10^{-4}$ ($p=0.848$) — validates labor-regulation channel.
...
\textbf{Outlier-excluded:} $\beta = -0.0206$ ($p=0.329$) — null holds. \\
\textbf{Median wage:} $\beta = -0.0477$ ($p=0.357$) — null holds.
...
\textbf{Verified Result:} $\beta_1 = 0.8503$ (SE $= 0.041$, $p<0.001$, N=101). \\
\textbf{Path dependence is 16.6$\times$ the formal productivity effect} ($\beta_2 = -0.051$). \\
95\% BCa Bootstrap CI: $[0.734,\; 0.968]$ — firmly above 0.5 structural lock-in threshold. \\
AR(2) lag: $\beta = +0.111$ ($p=0.269$) — non-significant, AR(1) specification appropriate.

\subsection{Test 4: Cost-Shifting (Price Squeeze)}
\textbf{Step 3 (Raw Mediation):} $\beta(Q_{\text{out}} \to w_{\text{inf}}) = -7.226$ ($p=0.009$) \\
\textbf{Step 6 (Orthogonalized Mediation):} $b = -3.919$ ($p=0.005$) \\
...
$\beta(\ln A_F \to N_{\text{inf}}) = -0.131$ ($p=0.257$) — \textbf{Inconclusive} (small-sample, low power).
...
\subsection*{Key Diagnostic Interpretations}
\begin{itemize}
    \item \textbf{Heteroscedasticity:} All 3 models are heteroscedastic under BP. This is expected and already addressed: all regressions in the dissertation use state-clustered standard errors via \texttt{fixest::feols()}, which is robust to arbitrary within-cluster correlation and heteroscedasticity. The BP test is reported for transparency only.
    \item \textbf{Autocorrelation:} All 3 Durbin-Watson statistics are close to 2.0, and $p$-values are well above 0.05. No autocorrelation is detected. This supports standard OLS-type inference.
    \item \textbf{Normality:} The outsourcing and persistence residuals are non-normal, likely due to the panel's short time dimension and extreme-valued observations. With cluster-robust inference (asymptotic), exact normality is not required for valid $t$-statistics. The wage pass-through residuals are approximately normal ($p=0.172$).
    \item \textbf{Oster Sensitivity Bounds:} Robustness Value ($RV$) = 7.6\%. An unobserved confounder explaining 7.6\% of both residual outsourcing variation and formal productivity variation would fully overturn the core elasticity. This is modest but indicates the result is not trivially fragile.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Bootstrap CI:} $[0.734,\; 0.968]$ — confirms $\beta_1 > 0.5$ (structural lock-in) and rules out $\beta_1 = 0$ (no persistence).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{figure\_cost\_shifting.png:} Partial correlation plot — orthogonalized excess outsourcing vs.\ informal wages. Negative slope ($b=-3.919$) confirms price squeeze.
    \item \textbf{figure\_adverse\_incorporation.png:} Formal productivity vs.\ informal enterprise count. Noisy, slightly negative relationship ($p=0.257$). Inconclusive due to compositional bias.
\end{itemize}
