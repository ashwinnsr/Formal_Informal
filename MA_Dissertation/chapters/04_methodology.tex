...
I construct a \textbf{State-Industry Panel} by pooling data from five cross-sectional rounds covering the period \textbf{2010--11 to 2023--24}. This includes historical microdata from the \textbf{NSS 67th Round (2010--11)} and \textbf{NSS 73rd Round (2015--16)} on Unincorporated Non-Agricultural Enterprises, alongside the modern \textbf{ASUSE series (2021--22 to 2023--24)}. The unit of observation is the state $\times$ NIC-2-digit industry cell rather than the district, reflecting the geographic resolution available in public microdata, which suppresses district identifiers to protect factory confidentiality.
...
A limitation of this enforcement measure is its time-invariance: $E_s$ reflects the 2023 enforcement landscape applied uniformly across the 2010--2024 panel. This assumes that the \textit{relative} enforcement ranking of states has remained stable over the observation period. While India's state-level labor enforcement rankings are broadly persistent---states such as Kerala and West Bengal consistently rank among the most enforcement-intensive, while Gujarat and Rajasthan consistently rank lowest---individual state trajectories can shift with changes in political leadership or industrial policy. This measurement error is attenuated by the fact that enforcement enters the regression as a cross-sectional moderator (interacted with productivity) rather than as a time-varying treatment, but it nonetheless introduces noise that biases interaction coefficients toward zero. The implications of this limitation are discussed further in Section~\ref{sec:limitations_discussion}.
...
I address this through a triangulation strategy. The triangulation relies on: (i) industry fixed effects absorbing time-invariant industry-level confounders; (ii) year fixed effects in the pooled specification absorbing common macro shocks; (iii) the placebo test in chemicals confirming that the outsourcing result is sector-specific rather than reflecting a generic productivity-enforcement correlation; (iv) robustness across six enforcement specifications; and (v) an IV/2SLS specification using lagged enforcement ($E_{s,t-1}$) as an instrument for contemporaneous enforcement, which provides a partial check on enforcement endogeneity (results reported in Chapter~\ref{ch:discussion}). I emphasize that the resulting estimates are conditional correlations consistent with the adverse incorporation hypothesis, not identified causal effects.
